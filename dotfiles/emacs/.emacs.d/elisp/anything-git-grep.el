...
(require 'screen-vc) ;; XXX featurep
...
(defun anything-git-submodule-grep-process ()
  (anything-aif (anything-attr 'default-directory)
      (let ((default-directory it))
        (start-process-shell-command ;; XXX compile warnings argsments?
...
(defvar anything-c-source-git-grep
  '((name . "Git Grep")
    (init . (lambda () (anything-attrset 'default-directory (screen-vc-current-toplevel))))
...
(defvar anything-c-source-git-submodule-grep
  '((name . "Git Submodule Grep")
    (init . (lambda () (anything-attrset 'default-directory (screen-vc-current-file-root))))
